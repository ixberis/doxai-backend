...
# =============================================================================
# DummyFacade para cursor pagination
# =============================================================================
...
class DummyFacade:
    ...
    def __init__(self, default_user_id: UUID):
    ...
    def _init_seed_events(self):
        ...
        self._events = [
            ...
            },
            {
                ...
                "user_id": self.default_user_id,
                "user_email": "test@example.com",
                "event_type": ProjectFileEvent.MOVED.value,  # "moved" (válido, no "downloaded")
...
class InMemoryProjectsQueryService:
    ...
    def list_projects_by_user(
        ...
        """
        items = [p for p in self._projects if str(p["user_id"]) == str(user_id) or p["user_id"] == user_id]
    ...
    def list_ready_projects(
        ...
        if user_id is not None:
            items = [p for p in items if str(p["user_id"]) == str(user_id) or p["user_id"] == user_id]
    ...
    def list_active_projects(
        ...
        *,
        ...
        items = [
            ...
            and (str(p["user_id"]) == str(user_id) or p["user_id"] == user_id)
        ]
    ...
    def list_file_events(
        ...
        items = self.facade.list_file_events(
            project_id=project_id,
            file_id=file_id,
...
class InMemoryProjectsCommandService:
    ...
    def update_project(
        self, project_id: UUID, user_id: str, user_email: str, **kwargs
    ) -> Dict[str, Any]:
        p = self._store.get(project_id) or InMemoryProjectsQueryService(
    ...
    def change_status(
        self, project_id: UUID, user_id: str, user_email: str, new_status: ProjectStatus
    ) -> Dict[str, Any]:
        p = self._store.get(project_id) or InMemoryProjectsQueryService(
            UUID(user_id) if isinstance(user_id, str) else user_id
        ).get_project_by_id(project_id)
    ...
    def transition_state(
        self, project_id: UUID, user_id: str, user_email: str, to_state: ProjectState
    ) -> Dict[str, Any]:
        p = self._store.get(project_id) or InMemoryProjectsQueryService(
            UUID(user_id) if isinstance(user_id, str) else user_id
        ).get_project_by_id(project_id)
    ...
    def move_file(self, file_id: UUID, user_id: str, user_email: str, new_path: str):
