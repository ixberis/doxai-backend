...
# -*- coding: utf-8 -*-
"""
backend/app/modules/payments/routes/webhooks_stripe.py
...
async def stripe_webhook(
    ...
    """
    Webhook de Stripe para notificar pagos y refunds.
    ...
    observe_webhook_received("stripe")
    ...
    try:
        ...
        observe_webhook_verified("stripe", verification.value, duration)
        observe_webhook_outcome("stripe", outcome.value)
        
        return result
        
    except WebhookSignatureError as e:
        duration = time.time() - start_time
        observe_webhook_rejected("stripe", "invalid_signature")
        WEBHOOKS_PROCESSING_SECONDS.labels(provider="stripe").observe(duration)
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=str(e))
    except Exception as e:
        duration = time.time() - start_time
        observe_webhook_rejected("stripe", "processing_error")
        WEBHOOKS_PROCESSING_SECONDS.labels(provider="stripe").observe(duration)
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))


# Fin del archivo backend/app/modules/payments/routes/webhooks_stripe.py
